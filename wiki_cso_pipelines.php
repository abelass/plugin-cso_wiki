...
function wiki_cso_formulaire_charger($flux){
    ...
    if ($form =='editer_article' AND !_request('exec')){
        ...
        if(is_array($rubriques_wiki))$sql=sql_select('id_article,id_rubrique,titre,soustitre','spip_articles','statut='.sql_quote('publie').' AND id_rubrique IN ('.implode(',',$rubriques_wiki).') AND lang='.sql_quote($lang).' AND id_article!=400 AND id_trad !=618','','titre');
